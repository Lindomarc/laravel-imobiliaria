$(function () {
    ...
    if (typeof($('select[name="civil_status"]')!== undefined)){
        function changeCivilStatus(){
            let civil_status = $('select[name="civil_status"] :selected').val()
            if (civil_status === '1' || civil_status === '2'){
                $('.content_spouse input').prop('disabled', false)
                $('.content_spouse select').prop('disabled', false)
            } else {
                $('.content_spouse input').prop('disabled', true)
                $('.content_spouse select').prop('disabled', true)
            }
        }
        changeCivilStatus()
        $('select[name="civil_status"]').change(()=>{
            changeCivilStatus()
        })
    }
